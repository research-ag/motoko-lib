...
actor {
    ...
    public query func profile32() : async (Nat64, Nat64, Nat64, Nat64, Nat64) {
        let a = E.countInstructions(func(){e32.process_block(block64)});
        let b = E.countInstructions(func(){ignore Sha2.fromBlob(#sha256, block0)});
        let c = E.countInstructions(func(){ignore Sha2.fromBlob(#sha256, block48)});
        let sha256 = Sha2.Digest(#sha256);
        let d = E.countInstructions(func(){ignore sha256.sum()});
        sha256.reset();
        let e = E.countInstructions(func(){ignore sha256.write(block48.vals()); ignore sha256.sum()});
        (a, b, c, d, e)
    };
